...
const GridItem = ({ header, href, onSelect, src, subheader, tech_stack }) => {
    return (
        <div
            className="w-full aspect-[calc(4/5)] bg-charcoal rounded-md overflow-hidden hover:scale-105 transition duration-200 ease-in-out cursor-pointer"
            onClick={() => onSelect(href)}
        >
            <Img src={src} className={"w-full h-2/3"} objectFit="cover" />
            <div className="col ml-5">
                <span className="font-bold 2xl:text-2xl xl:text-xl lg:text-lg sm:text-lg text-2xl 2xl:mt-8 sm:mt-4 mt-4">
                    {header}
                </span>
